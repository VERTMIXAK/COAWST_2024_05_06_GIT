...
#        now=`date -d $dum "+%Y%m%d"`
        ...
        do
            ...
            part2="$now/nam_218_"
            part3=$now"_1800_0"${hours[$ii]}".grb2?"
            part4="var=$longName&north=$latMax&west=$lonMin&east=$lonMax&south=$latMin&disableProjSubset=on&horizStride=1&time_start="

			newHour=` echo ${hours[$ii]} + 18 | bc`
			dumDate=`date -d "$now + $newHour hour" +%Y-%m-%d`
			dumHour=`date -d "$now + $newHour hour" +%H`
			datePlus=$dumDate"T"$dumHour"%3A00%3A00"
...
#			    source ~/.runPycnal
			    python settime18.py $outFile $time $baseDays
#			    source ~/.runROMSintel
