...
sfos.myGridFile = ['../DOPPIOdata_parent/grid_DOPPIO.nc'];

[dum1,dum2]=unix('pwd | rev |cut -d "/" -f2 | rev | cut -d "_" -f1');
sfos.myPrefix = lower(dum2(1:end-1));

HISfiles  = dir('../DOPPIOdata_parent/DOPPIO_*');
% HIS2files = dir('../DOPPIOdata_parent/*_his2_*');
HIS2files = HISfiles;

sfos.grid = roms_get_grid(['../DOPPIOdata_parent/',HISfiles(1).name],sfos.myGridFile,0,1);
sfos.grid.pm = nc_varget(sfos.myGridFile,'pm');
sfos.grid.pn = nc_varget(sfos.myGridFile,'pn');
...
% % % calculated above
% % Cs_r = nc_varget(['../DOPPIOdata_parent/',HISfiles(1).name],'Cs_r');
% % Cs_w = nc_varget(['../DOPPIOdata_parent/',HISfiles(1).name],'Cs_w');
% % 
% % C_w - Cs_w'
% % 
...
% % s_w = nc_varget(['../DOPPIOdata_parent/',HISfiles(1).name],'s_w');
% % s_r = nc_varget(['../DOPPIOdata_parent/',HISfiles(1).name],'s_rho');
% % 
% % % Calculate S(sigma)
...
for tt=1:Nt
    ...
    [dum1,dum2] = roms_get_date(['../DOPPIOdata_parent/',HISfiles(tt).name]);
    sfos.time(tt)       = dum1(1);
    
    % read in the surface fields from the netcdf file.
    
    SSH_rho    = sq(nc_varget(['../DOPPIOdata_parent/',HIS2files(tt).name],'zeta',[0,0,0],[1,-1,-1]) )    .* sfos.grid.mask_rho;
    shflux_rho = sq(nc_varget(['../DOPPIOdata_parent/',HIS2files(tt).name],'shflux',[0,0,0],[1,-1,-1]) )  .* sfos.grid.mask_rho;
    ssflux_rho = sq(nc_varget(['../DOPPIOdata_parent/',HIS2files(tt).name],'ssflux',[0,0,0],[1,-1,-1]) )  .* sfos.grid.mask_rho;
    %     swrad_rho  = sq(nc_varget(['../DOPPIOdata_parent/',HIS2files(tt).name],'swrad',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_rho;    
    sustr_u    = sq(nc_varget(['../DOPPIOdata_parent/',HIS2files(tt).name],'sustr',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_u;
    svstr_v    = sq(nc_varget(['../DOPPIOdata_parent/',HIS2files(tt).name],'svstr',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_v;
...
%         1 + ( 1 + sfos.grid.h(myJ,myI) ) * SofSigma
    ...
    Tdz_rho  = sq(nc_varget(['../DOPPIOdata_parent/',HISfiles(tt).name],'temp',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.grid.mask_rho3D .* sfos.grid.dz_rho;
    Sdz_rho  = sq(nc_varget(['../DOPPIOdata_parent/',HISfiles(tt).name],'salt',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.grid.mask_rho3D .* sfos.grid.dz_rho;
