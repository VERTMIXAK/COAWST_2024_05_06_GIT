...
sfos.myGridFile = ['../../../InputFiles/Gridpak_child/NGnest_100m_child.nc'];

[dum1,dum2]=unix('pwd | rev |cut -d "/" -f2 | rev | cut -d "_" -f1');
sfos.myPrefix = lower(dum2(1:end-1));

HISfiles  = dir('../netcdfOutput_split_child/HIS_*');
HIS2files = dir('../netcdfOutput_split_child/HIS2_*');

sfos.grid = roms_get_grid(sfos.myGridFile,['../netcdfOutput_split_child/',HISfiles(1).name],0,1);
...
% % % calculated above
% % Cs_r = nc_varget(['../netcdfOutput_split_child/',HISfiles(1).name],'Cs_r');
% % Cs_w = nc_varget(['../netcdfOutput_split_child/',HISfiles(1).name],'Cs_w');
% % 
% % C_w - Cs_w'
% % 
...
% % s_w = nc_varget(['../netcdfOutput_split_child/',HISfiles(1).name],'s_w');
% % s_r = nc_varget(['../netcdfOutput_split_child/',HISfiles(1).name],'s_rho');
% % 
% % % Calculate S(sigma)
...
% (apchildly) the rho-grid tile implied by (pm(J,I), pn(J,I)) extends to
...
% NOTE: This script assumes 1 snapshot per file

for tt=1:Nt
    
    [dum1,dum2] = roms_get_date(['../netcdfOutput_split_child/',HIS2files(tt).name]);
    sfos.timeHIS2(tt)       = dum1(1);
    
    % read in the surface fields from the netcdf file.
    
    SSH_rho    = sq(nc_varget(['../netcdfOutput_split_child/',HIS2files(tt).name],'zeta',[0,0,0],[1,-1,-1]) )    .* sfos.grid.mask_rho;
    shflux_rho = sq(nc_varget(['../netcdfOutput_split_child/',HIS2files(tt).name],'shflux',[0,0,0],[1,-1,-1]) )  .* sfos.grid.mask_rho;
    ssflux_rho = sq(nc_varget(['../netcdfOutput_split_child/',HIS2files(tt).name],'ssflux',[0,0,0],[1,-1,-1]) )  .* sfos.grid.mask_rho;
    %     swrad_rho  = sq(nc_varget(['../netcdfOutput_split_child/',HIS2files(tt).name],'swrad',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_rho;    
    sustr_u    = sq(nc_varget(['../netcdfOutput_split_child/',HIS2files(tt).name],'sustr',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_u;
    svstr_v    = sq(nc_varget(['../netcdfOutput_split_child/',HIS2files(tt).name],'svstr',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_v;
    SST_rho    = sq(nc_varget(['../netcdfOutput_split_child/',HIS2files(tt).name],'temp_sur',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_rho;
    SSS_rho    = sq(nc_varget(['../netcdfOutput_split_child/',HIS2files(tt).name],'salt_sur',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_rho;
...
%     
% 
%     Tdz_rho  = sq(nc_varget(['../netcdfOutput_split_child/',HISfiles(tt).name],'temp',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.grid.mask_rho3D .* sfos.grid.dz_rho;
%     Sdz_rho  = sq(nc_varget(['../netcdfOutput_split_child/',HISfiles(tt).name],'salt',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.grid.mask_rho3D .* sfos.grid.dz_rho;
...
%     done('reading in data')  
    ...
    SST_rho .* sfos.grid.mask_rho ./ sfos.grid.pm ./ sfos.grid.pn;
    sfos.SST_BasinAve(tt) = nansum(ans(:)) / sfos.basinArea;
    
    SSS_rho .* sfos.grid.mask_rho ./ sfos.grid.pm ./ sfos.grid.pn;
...
% NOTE: this script assumes one snapshot per file


for tt=1:Nt
    
    [dum1,dum2] = roms_get_date(['../netcdfOutput_split_child/',HISfiles(tt).name]);
    sfos.timeHIS(tt)       = dum1(1);
   

    % Calculate the dynamic water volume
    
    SSH_rho    = sq(nc_varget(['../netcdfOutput_split_child/',HISfiles(tt).name],'zeta',[0,0,0],[1,-1,-1]) )    .* sfos.grid.mask_rho;
...
fig(20);plot(sfos.timeHIS,sfos.grid.basinVolume/10^9);title('Total basin water volume (km^3)');;
datetick('x','mm/dd','keepticks')
print('figures/Temp_BasinAve','-djpeg');

fig(1);plot(sfos.timeHIS,sfos.Temp_BasinAve);title('Basin-average temperature');;
datetick('x','mm/dd','keepticks')
print('figures/Temp_BasinAve','-djpeg');

fig(2);plot(sfos.timeHIS,sfos.Salt_BasinAve);title('Basin-average salinity');
datetick('x','mm/dd','keepticks')
print('figures/Salt_BasinAve','-djpeg');

fig(3);plot(sfos.timeHIS2,sfos.SSH_BasinAve);title('Basin-average zeta');
datetick('x','mm/dd','keepticks')
print('figures/SSH_BasinAve' ,'-djpeg');

fig(4);plot(sfos.timeHIS2,sfos.SST_BasinAve);title('Basin-average surface temperature');
datetick('x','mm/dd','keepticks')
print('figures/SST_BasinAve' ,'-djpeg');

fig(5);plot(sfos.timeHIS2,sfos.SSS_BasinAve);title('Basin-average surface salinity');
datetick('x','mm/dd','keepticks')
print('figures/SSS_BasinAve' ,'-djpeg');



fig(6);plot(sfos.timeHIS2,sfos.shflux_BasinAve);title('Basin-average shflux');
datetick('x','mm/dd','keepticks')
print('figures/shflux_BasinAve' ,'-djpeg');

fig(7);plot(sfos.timeHIS2,sfos.ssflux_BasinAve);title('Basin-average ssflux');
datetick('x','mm/dd','keepticks')
...
fig(9);plot(sfos.timeHIS2,sfos.sustr_BasinAve);title('Basin-average sustr');
datetick('x','mm/dd','keepticks')
print('figures/sustr_BasinAve' ,'-djpeg');

fig(10);plot(sfos.timeHIS2,sfos.svstr_BasinAve);title('Basin-average svstr');
datetick('x','mm/dd','keepticks')
