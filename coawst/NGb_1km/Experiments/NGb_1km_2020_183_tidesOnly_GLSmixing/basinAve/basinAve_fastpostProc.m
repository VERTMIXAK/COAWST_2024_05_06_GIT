...
% % % calculated above
% % Cs_r = nc_varget(['../netcdfOutput/',HISfiles(1).name],'Cs_r');
% % Cs_w = nc_varget(['../netcdfOutput/',HISfiles(1).name],'Cs_w');
% % 
% % C_w - Cs_w'
% % 
...
% % s_w = nc_varget(['../netcdfOutput/',HISfiles(1).name],'s_w');
% % s_r = nc_varget(['../netcdfOutput/',HISfiles(1).name],'s_rho');
% % 
% % % Calculate S(sigma)
...
%         1 + ( 1 + sfos.grid.h(myJ,myI) ) * SofSigma
    ...
    % You can also calculate the z_w with a canned function but it reverses
    % the order of the indices
%     myz_w = set_depth(sfos.grid.Vtransform,sfos.grid.Vstretching,sfos.grid.theta_s,sfos.grid.theta_b,sfos.grid.hc,sfos.grid.N,5,sfos.grid.h',SSH_rho');
    ...
    Tdz_rho  = sq(nc_varget(['../netcdfOutput/',HISfiles(tt).name],'temp',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.grid.mask_rho3D .* sfos.grid.dz_rho;
    Sdz_rho  = sq(nc_varget(['../netcdfOutput/',HISfiles(tt).name],'salt',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.grid.mask_rho3D .* sfos.grid.dz_rho;
...
fig(20);plot(sfos.time,sfos.grid.basinVolume/10^9);title('Total basin water volume (km^3)');xlabel('date -   2016');
