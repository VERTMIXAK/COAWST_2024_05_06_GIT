...
dstgrd = pycnal.grid.get_ROMS_grid('NGb_1km')
...
for typ in type:
    for tip in type:
        ...
        grid2_file = 'remap_grid_NGb_1km_'+str(tip)+'.nc'
        interp_file1 = 'remap_weights_DOPPIO_to_NGb_1km_bilinear_'+str(typ)+'_to_'+str(tip)+'.nc'
        interp_file2 = 'remap_weights_NGb_1km_to_DOPPIO_bilinear_'+str(tip)+'_to_'+str(typ)+'.nc'
        map1_name = 'DOPPIO to NGb_1km Bilinear Mapping'
        map2_name = 'NGb_1km to DOPPIO Bilinear Mapping'
        num_maps = 1
        map_method = 'bilinear'
