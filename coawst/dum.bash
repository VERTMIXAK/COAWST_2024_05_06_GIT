...
#exptMode="tidesOnly"
#exptMode="island"
#exptMode="tidesOnly_GLSmixing"
#exptMode="deadStill_noSalinity"
#exptMode="deadStill"
#exptMode="deadStill_GLSmixing"
#exptMode="deadStill_LMDmixing"
...
#nameAppend="_twoWay"
#nameAppend="_MERRA"
#nameAppend="_CMEMS_GFS_proto"
#nameAppend="_CMEMS_GFS"
nameAppend="_protoStations"
...
forceTair=`grep Tair $runInputFile | grep -v '^!' | grep nc | head -1 | rev |cut -d '/' -f1| rev | cut -d '_' -f1`
echo "forceTair " $forceTair

forceQair=`grep Qair $runInputFile | grep -v '^!' | grep nc | head -1 | rev |cut -d '/' -f1| rev | cut -d '_' -f1`
echo "forceQair " $forceQair

forceWind=`grep Uwind $runInputFile | grep -v '^!' | grep nc | head -1 | rev |cut -d '/' -f1| rev | cut -d '_' -f1`
...
#	echo "nflt = $nflt"
	...
	fltName=`echo "./"$gridName"/"``grep 'FPOSNAM =' $runInputFile | rev |cut -d ' ' -f1 | rev | cut -d '/' -f3-10`
	#echo $fltName
...
#    echo 'fi'																							>>$batName
    ...
    echo "bash /import/home/jgpender/addl_Scripts/timeROMS/getRunTime.bash >> log" 						>>$batName
