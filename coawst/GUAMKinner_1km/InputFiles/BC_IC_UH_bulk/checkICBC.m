...
fig(20);clf;
plot(myZ(end-range:end),mySalt(end-range:end));title('salt')

fig(21);clf;
plot(myZ(end-range:end),myTemp(end-range:end));title('temp')


pden = sw_pden(mySalt,myTemp,myZ,0);

fig(22);clf;
plot(myZ(end-range:end),pden(end-range:end));title('density')

fig(23);clf;
plot(myZ(end-range:end),diff(pden(end-range-1:end)));title('diff density')
