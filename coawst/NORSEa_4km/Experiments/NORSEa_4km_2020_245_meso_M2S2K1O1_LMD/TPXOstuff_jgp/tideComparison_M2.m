...
[~,dum] =  unix('ls ../netcdfOutput_days001_030/*_his_* | head -1'); model.HISfile = dum(1:end-1)
...
[~,dum] = unix('ls ../netcdfOutput_days001_030 | tail -1 | cut -d "_" -f1');model.file_prefix = [dum(1:end-1),'_']
model.file_suffix = '';
...
if ~exist(outfile)
    ...
    if lon0TPXO < lon1TPXO
        ...
        tpxo.phy_zeta = tpxo.pha_zeta(jTPXO,iTPXO);
...
%% Harper's figure of merit
    ...
    num=(model.amp_zeta - tpxo.amp_zeta_BB).^2 .*model.dA_zeta.*model.mask_rho;
    den=model.dA_zeta.*model.mask_rho;
...
pcolor(model.lon_v,model.lat_v,tpxo.pha_vbar_VV);shading flat;
caxis(phaLim);
rect;hold on
