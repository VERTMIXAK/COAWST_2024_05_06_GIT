...
#
##today=`date -d "yesterday -3 days" +"%Y%m%d"`
#today=`date -d "yesterday" +"%Y%m%d"`
#echo $today

##!!!!!!! new scheme for 'today'
# 'today' is now the first time stamp in the 'fore' file

rawDate=`head -1 ../today.txt`
dateInDays=`echo " ( $rawDate + 3155673600 ) / 86400 " | bc`
echo $rawDate $dateInDays
today=`grep ",$dateInDays," ~/addl_Scripts/dayConverterCommas.txt | cut -d ',' -f1`
echo $today
...
/bin/bash /import/c1/VERTMIX/jgpender/coawst/PALAU_800m/InputFiles/automation/GFS_PALAU_forecast/fixNamesTimes.bash  > log.txt
