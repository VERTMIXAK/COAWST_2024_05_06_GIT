...
HISfiles  = dir('../netcdfOutput/*_his_*');
HIS2files = dir('../netcdfOutput/*_his2_*');

sfos.grid = roms_get_grid(sfos.myGridFile,['../netcdfOutput/',HISfiles(1).name],0,1);
...
for tt=1:Nt
    ...
    [dum1,dum2] = roms_get_date(['../netcdfOutput/',HISfiles(tt).name]);
    sfos.time(tt)       = dum1(1);
    dum2(1)
    
    % read in the fields from the netcdf file.
    
    Tdz_rho  = sq(nc_varget(['../netcdfOutput/',HISfiles(tt).name],'temp',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.mask_rho3D .* sfos.dz_rho;
    Sdz_rho  = sq(nc_varget(['../netcdfOutput/',HISfiles(tt).name],'salt',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.mask_rho3D .* sfos.dz_rho;   
  
    SSH_rho    = sq(nc_varget(['../netcdfOutput/',HIS2files(tt).name],'zeta',[0,0,0],[1,-1,-1]) )    .* sfos.grid.mask_rho;   
    shflux_rho = sq(nc_varget(['../netcdfOutput/',HIS2files(tt).name],'shflux',[0,0,0],[1,-1,-1]) )  .* sfos.grid.mask_rho;
    ssflux_rho = sq(nc_varget(['../netcdfOutput/',HIS2files(tt).name],'ssflux',[0,0,0],[1,-1,-1]) )  .* sfos.grid.mask_rho;
    swrad_rho  = sq(nc_varget(['../netcdfOutput/',HIS2files(tt).name],'swrad',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_rho;
    
    sustr_u    = sq(nc_varget(['../netcdfOutput/',HIS2files(tt).name],'sustr',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_u;
    svstr_v    = sq(nc_varget(['../netcdfOutput/',HIS2files(tt).name],'svstr',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_v;
...
fig(1);plot(sfos.time,sfos.Temp_BasinAve);title('Basin-average temperature (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/Temp_BasinAve','-djpeg');

fig(2);plot(sfos.time,sfos.Salt_BasinAve);title('Basin-average salinity (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/Salt_BasinAve','-djpeg');

fig(3);plot(sfos.time,sfos.SSH_BasinAve);title('Basin-average zeta (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/SSH_BasinAve' ,'-djpeg');

fig(4);plot(sfos.time,sfos.SST_BasinAve);title('Basin-average surface temperature (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/SST_BasinAve' ,'-djpeg');

fig(5);plot(sfos.time,sfos.SSS_BasinAve);title('Basin-average surface salinity (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/SSS_BasinAve' ,'-djpeg');



fig(6);plot(sfos.time,sfos.shflux_BasinAve);title('Basin-average shflux (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/shflux_BasinAve' ,'-djpeg');

fig(7);plot(sfos.time,sfos.ssflux_BasinAve);title('Basin-average ssflux (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/ssflux_BasinAve' ,'-djpeg');

fig(8);plot(sfos.time,sfos.swrad_BasinAve);title('Basin-average swrad (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/swrad_BasinAve' ,'-djpeg');

fig(9);plot(sfos.time,sfos.sustr_BasinAve);title('Basin-average sustr (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
print('figures/sustr_BasinAve' ,'-djpeg');

fig(10);plot(sfos.time,sfos.svstr_BasinAve);title('Basin-average svstr (GFS)');xlabel('date -   2016')
datetick('x','mm/dd','keepticks')
