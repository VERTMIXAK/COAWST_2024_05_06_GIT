...
def remap_bdry_uv(src_file, src_grd, dst_grd, dxy=20, cdepth=0, kk=2, dst_dir='./'):
    ...
    src_uz = pycnal_toolbox.Grid_HYCOM.flood_fast(src_varu, src_grd, pos='t', \
                spval=spval, dxy=dxy, cdepth=cdepth, kk=kk)
    src_vz = pycnal_toolbox.Grid_HYCOM.flood_fast(src_varv, src_grd, pos='t', \
                spval=spval, dxy=dxy, cdepth=cdepth, kk=kk)
    ...
    # rotate u,v fields
    src_angle = pycnal.remapping.remap(src_grd.angle, './ini_074_parent/remap_weights_GLBa0.08_to_PRAIRIE_3km_1to1_parent_sphFALSE_bilinear_t_to_rho.nc', \
                                      spval=spval)
