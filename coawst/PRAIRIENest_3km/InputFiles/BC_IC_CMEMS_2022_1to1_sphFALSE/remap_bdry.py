...
#    #print(src_file)
    ...
    if os.path.exists(dst_file) is True:
        os.remove(dst_file)
    pycnal_toolbox.nc_create_roms_bdry_file(dst_file, dst_grd, nctime)
    ...
    if src_varname == 'ssh':
        ...
        wts_file = './ini_074_parent/remap_weights_GLBa0.08_to_PRAIRIE_3km_1to1_parent_sphFALSE_bilinear_t_to_rho.nc'
        dst_varname = 'zeta'
...
# JGP experiment  !!!!!!!!!!!!
    ...
    #print('about to run remap')
#    #print('src_varz is ', src_varz)
    #print('wts_file is ', wts_file)
#    #print('spval is ', spval)



    dst_varz = pycnal.remapping.remap(src_varz, wts_file, spval=spval)


    #print('about to do i ndim statement')


    if ndim == 3:
        # vertical interpolation from standard z level to sigma
        #print('vertical interpolation from standard z level to sigma'
        dst_var_north = pycnal.remapping.z2roms(dst_varz[::-1, Mp-1:Mp, :], \
                          dst_grdz, dst_grd, Cpos=Cpos, spval=spval, \
                          flood=False, irange=(0,Lp), jrange=(Mp-1,Mp))
        dst_var_south = pycnal.remapping.z2roms(dst_varz[::-1, 0:1, :], \
                          dst_grdz, dst_grd, Cpos=Cpos, spval=spval, \
                          flood=False, irange=(0,Lp), jrange=(0,1))
        dst_var_east = pycnal.remapping.z2roms(dst_varz[::-1, :, Lp-1:Lp], \
                          dst_grdz, dst_grd, Cpos=Cpos, spval=spval, \
                          flood=False, irange=(Lp-1,Lp), jrange=(0,Mp))
        dst_var_west = pycnal.remapping.z2roms(dst_varz[::-1, :, 0:1], \
                          dst_grdz, dst_grd, Cpos=Cpos, spval=spval, \
