...
% !!!!!!  NOTE   !!!!!!!!
%
% roms_get_grid doens't work with IC files. You have to use an actual ROMS
% output file.

HISfiles  = dir('../../../InputFiles/BC_IC_CMEMS_2020/fullSet/GL*');
% HIS2files = dir('../../../InputFiles/BC_IC_CMEMS_2020/fullSet/GL*');
romsOutputFile = '../netcdfOutput/sd_his_00001.nc';

sfos.grid = roms_get_grid(sfos.myGridFile,romsOutputFile,0,1);
...
for tt=1:Nt
    ...
    [dum1,dum2] = roms_get_date(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HISfiles(tt).name]);
    sfos.time(tt)       = dum1(1);
    dum2(1)
    
    % read in the fields from the netcdf file.
    
    Tdz_rho  = sq(nc_varget(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HISfiles(tt).name],'temp',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.mask_rho3D .* sfos.dz_rho;
    Sdz_rho  = sq(nc_varget(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HISfiles(tt).name],'salt',[0,0,0,0],[1,-1,-1,-1]) )  .* sfos.mask_rho3D .* sfos.dz_rho;   
  
    SSH_rho    = sq(nc_varget(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HISfiles(tt).name],'zeta',[0,0,0],[1,-1,-1]) )    .* sfos.grid.mask_rho;   
%     shflux_rho = sq(nc_varget(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HIS2files(tt).name],'shflux',[0,0,0],[1,-1,-1]) )  .* sfos.grid.mask_rho;
%     ssflux_rho = sq(nc_varget(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HIS2files(tt).name],'ssflux',[0,0,0],[1,-1,-1]) )  .* sfos.grid.mask_rho;
%     swrad_rho  = sq(nc_varget(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HIS2files(tt).name],'swrad',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_rho;
%     
%     sustr_u    = sq(nc_varget(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HIS2files(tt).name],'sustr',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_u;
%     svstr_v    = sq(nc_varget(['../../../InputFiles/BC_IC_CMEMS_2020/fullSet/',HIS2files(tt).name],'svstr',[0,0,0],[1,-1,-1]) )   .* sfos.grid.mask_v;
