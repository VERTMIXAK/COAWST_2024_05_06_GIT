h = nc_varget('HC_125mME.nc','h');
mask = nc_varget('HC_125mME.nc','mask_rho');
[ny,nx] = size(mask);
...
fig(1);clf;
imagesc(mask);axis xy;ylim([ny-5 ny]);xlim([nx-35 nx])

jgpRange = [275:304];
mask(end,jgpRange);
JGPwidth = length(ans)*150

% So both UW and I have 4500m as the width of the channel.

JGParea = sum( 150 .* h(end,jgpRange)  )
