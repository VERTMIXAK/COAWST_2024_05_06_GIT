...
fileU = 'u.nc'
fileV = 'v.nc'
gfile = 'GUAMHinner_1km_seamount.nc'
...
%  pcolor(grd.x_u/1e3,grd.y_u/1e3,squeeze(u_bc(48,:,:)));caxis([-1,1]/10);axis equal tight;shading flat
 pcolor(grd.x_u/1e3,grd.y_u/1e3,squeeze(u_bc(tt,:,:)));
 caxis([-1,1]/50);
 axis equal tight;shading flat;title('u')
 pause(.1)
end;
...
figure(2);clf;colormap(gray)
 x = grd.x_u(myJ,:)/1e3;y= 1:nt;dat = squeeze(u_bc(1:nt,myJ,:));whos x y dat
...
for tt=1:50
    tt
 plot(sq(u_bc(tt,myJ,1:round(nx/2))));title('u');ylim(.02*[-1 1])
 pause(.2)
end;
aaa=5;
