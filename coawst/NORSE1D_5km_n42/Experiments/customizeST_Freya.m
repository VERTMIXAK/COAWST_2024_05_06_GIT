...
gridFile = './util/NORSE1D_5km_n42.nc';
HISFile  = './util/IC.nc';
...
% This will only be the top 600m of the water column
%   
zLocal = sq(grd.z_r(:,1,1));
zLocalShort = zLocal(20:end);
TlocalShort = interp1(zcruise,Tcruise,zLocalShort );
SlocalShort = interp1(zcruise,Scruise,zLocalShort );

% fix NaNs near surface
nz = length(zLocalShort)
for kk=nz-5:nz
    if isnan(TlocalShort(kk))
        TlocalShort(kk) = TlocalShort(kk-1);
    end; 
    if isnan(SlocalShort(kk))
        SlocaShortl(kk) = SlocalShort(kk-1);
    end;
end;
...
Tlocal(20:end) = TlocalShort;
Slocal(20:end) = SlocalShort;
