...
iReleaseLocs = [iLL:iUR]
jReleaseLocs = jLL*ones(1,length(iReleaseLocs))

count = length(iReleaseLocs)

myLat = 0*iReleaseLocs;
myLon = myLat;
for nn=1:count
    myLat(nn) = lat(jReleaseLocs(nn),iReleaseLocs(nn));
    myLon(nn) = lon(jReleaseLocs(nn),iReleaseLocs(nn));
end;
...
fileID = fopen( 'floats_HC_latLon.in','w');
...
for nn=1:length(iReleaseLocs);
    nn
    fprintf(fileID,'      1  1  1  %3i  0.0d0 %4i  %4i  -1  %16.8f 0.d0 0.d0 0.d0\n',nReleases,myLon(nn),myLat(nn),nInterval);          
end;
...
fig(5);clf;pcolor(lon,lat,mask);shading flat;hold on
plot(myLon,myLat,'ko')
