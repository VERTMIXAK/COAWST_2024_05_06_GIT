...
HISnames  = dir('../netcdfOutput_nameLinks/*_his_*') ;
HIS2names = dir('../netcdfOutput_nameLinks/*_his2_*');

gridFile    = '../HC_125mME_wetDry.nc';
newGridFile = 'grid.nc';

[nFiles,~] = size(HISnames);

dum = ['../netcdfOutput_nameLinks/',HISnames(2).name];
...
for ff=1:nFiles
    ...
    sourceHIS   = ['../netcdfOutput_nameLinks/',HISnames(ff).name];
    sourceHIS2  = ['../netcdfOutput_nameLinks/',HIS2names(ff).name];
    outHIS      = 'outHIS.nc';
    outHIS2     = 'outHIS2.nc';
    
    [~,fileDate] =  unix(['echo ',sourceHIS,' | rev | cut -d "_" -f1 | rev | cut -d "." -f1']);fileDate=fileDate(1:end-1);
    
    grd = roms_get_grid(gridFile,['../netcdfOutput_nameLinks/',HISnames(1).name],0,1);
