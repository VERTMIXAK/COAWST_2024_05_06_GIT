...
cp $Appsdir/WW3_config/ww3_grid.inp_template $Appsdir/WW3_config/ww3_grid.inp
cp $Appsdir/WW3_config/ww3_ounf.inp_template $Appsdir/WW3_config/ww3_ounf.inp
...
# get ROMS start date from ocean.in file
ini_name=`grep ININAME $Appsdir/ocean*.in | grep -v '!' | cut -d '=' -f3  `
...
startROMS=`grep ","$dstart"," ~/arch/addl_Scripts/dayConverterCommas.txt | cut -c 1-10 | sed "s/-//g"`
echo "startROMS = "$startROMS

# find length of run from ROMS input file
echo ""
ntimes=`grep "NTIMES ==" $Appsdir/ocean*.in | grep -v "!" | cut -d "=" -f3`
echo "ntimes = " $ntimes

dt=`grep "DT ==" $Appsdir/ocean*.in | grep -v "!" | cut -d "=" -f3 | cut -d "." -f1`
echo 'dt=' $dt
...
dEnd=` echo " $dstart + $runDays " | bc`
echo "dEnd = " $dEnd

endROMS=`grep ","$dEnd"," ~/arch/addl_Scripts/dayConverterCommas.txt | cut -c 1-10 | sed "s/-//g"`
echo "endROMS = "$endROMS
...
sed -i "s/myStartDate/$startDate/" $Appsdir/WW3_config/ww3_grid.inp
sed -i "s/myStartDate/$startDate/" $Appsdir/WW3_config/ww3_ounf.inp

sed -i "s/myEndDate/$endDate/" $Appsdir/WW3_config/ww3_shel.inp
sed -i "s/myEndDate/$endDate/" $Appsdir/WW3_config/ww3_grid.inp
sed -i "s/myEndDate/$endDate/" $Appsdir/WW3_config/ww3_ounf.inp

sed -i "s/\$\$\$\$[0-9]//g" $Appsdir/WW3_config/ww3_shel.inp
sed -i "s/\$\$\$\$[0-9]//g" $Appsdir/WW3_config/ww3_grid.inp
sed -i "s/\$\$\$\$[0-9]//g" $Appsdir/WW3_config/ww3_ounf.inp



# make the WW3 time step 6 times larger than the ROMS time step for starters

dtWW3=` echo " $dt * 6 " | bc`

sed -i "s/timeStep/$dtWW3/g" $Appsdir/WW3_config/ww3_grid.inp
