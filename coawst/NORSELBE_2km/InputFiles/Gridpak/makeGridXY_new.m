...
% The longitude source data interval is about 2.4 km, which is close enough 
% to the 2km inner grid to ignore, but the latitude source data interval is
...
% y direction only
fullY =[ yPsiMin - fliplr(cumsum(myDiffs))    yCore    yPsiMax+cumsum(myDiffs)   ]
fig(1);clf;plot(diff(fullY))
fig(2);clf;plot(fullY)


fullX=xCore;
fig(3);clf;plot(diff(fullX))
