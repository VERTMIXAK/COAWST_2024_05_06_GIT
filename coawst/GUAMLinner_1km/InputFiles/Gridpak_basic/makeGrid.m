...
% NOTE: I am now using a new 4km grid from Brian, not the old 8km grid!
...
% interval goes from a steady ~8km to a linearly increasing interval. All of
% the GUAMXinner_1km have this feature because every 8th lon/lat point on
...
% So Brian's grid is about 4km so I use his grid points in the interior of
% my grid, but add 4 points between each of Brian's points to give a 1km
...
% A sigmoid function that transitions from 1km to 4km looks like this
%   1 + 7 / (1 + exp(-x) )
...
% How many Brian gridpoints to get about 207 km from the LHS?

nLHS=iStretchLL;     
nRHS=iStretchUR;   
geodesic_dist(lonFrinkSouth(1),latFrinkSouth(1),lonFrinkSouth(1+nLHS),latFrinkSouth(1+nLHS));
['recheck western transition zone ',num2str(ans),' km']
...
% How many Brian gridpoints to get about 141 km from the RHS?


nRHS=iStretchUR;
geodesic_dist(lonFrinkSouth(end-nRHS),latFrinkSouth(end-nRHS),lonFrinkSouth(end),latFrinkSouth(end));
['recheck eastern transition zone ',num2str(ans),' km']
...
%   lonFrinkSouth(pos8) to lonFrinkSouth(posEm6)
...
for nn=1+nLHS:length(myVar)-nRHS-1
    [myVar(nn):(myVar(nn+1)-myVar(nn))/deltaFrink:myVar(nn+1)];
