...
oldGrid = 'GUAMLinner_1km.nc_origBathy';
newGrid = 'GUAMLinner_1km_edgeSmoothing.nc';
newGrid2= 'GUAMLinner_1km.nc'

unix(['cp ',oldGrid,' ',newGrid]);
unix(['cp ',oldGrid,' ',newGrid2]);
...
hWork(1,:) = lowpass(hWork(1,:),window);
hWork(end,:) = lowpass(hWork(end,:),window);
hWork(:,1) = lowpass(hWork(:,1),window);
hWork(:,end) = lowpass(hWork(:,end),window);
