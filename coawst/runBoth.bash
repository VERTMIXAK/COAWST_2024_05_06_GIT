...
exptMode="mesoNoTides_LMD_ROMS_WW3"
...
couplingFile=$Appsdir"coupling.in"
...
printf "cp "$couplingFile_"$exptMode  \t$couplingFile\n"
cp $couplingFile"_template"     $couplingFile
...
nameAppend="_addlCoupling_KIRBY"
#nameAppend=""
...
if [ $useTides == '1' ]; then
	...
	tideCode=`grep TIDENAME $runInputFile | grep -v '^!'  | cut -d '_' -f2 `
	tideCode="_$tideCode"
else
	tideCode=""
fi
...
# update the OCAWST_APPLICATION variable
...
## compile
#sed -i '/USE_DEBUG ?= on/c\   USE_DEBUG ?= ' makefile
#make -j 8   > makelogM

echo "compile"
./coawst.bash > log.both
...
cp  $couplingFile					$exptName
cp -r $Appsdir/WW3_config   		$exptName/Apps
cp runBoth.bash 					$exptName/Apps
...
wallTime="24:00:00"
nProcROMS=` echo " $tileX * $tileY " | bc `
nProcWW3=$nProcROMS
nProc=` echo " $nProcROMS + $nProcWW3 " | bc`
...
#    echo '#SBATCH --partition=t2small'              													>>$batName 
    ...
    echo "mpirun -np $nProc  coawstROMSWW3 coupling.in > log"   										>>$batName 
...
# make coupling script
sed -i "s/XXX/$nProcROMS/" 		coupling.in
sed -i "s/YYY/$nProcWW3/" 		coupling.in
dum=`echo $runInputFile | rev | cut -d '/' -f1 | rev`
sed -i "s/ZZZ/Apps\/$dum/"	coupling.in
