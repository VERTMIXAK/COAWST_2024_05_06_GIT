...
eta_psi=`ncdump -h $file | grep 'eta_psi =' | cut -d ' ' -f3`
eta_u=`ncdump -h $file | grep 'eta_u =' | cut -d ' ' -f3`
eta_v=`ncdump -h $file | grep 'eta_v =' | cut -d ' ' -f3`

xi_psi=`ncdump -h $file | grep 'xi_psi =' | cut -d ' ' -f3`
xi_u=`ncdump -h $file | grep 'xi_u =' | cut -d ' ' -f3`
xi_v=`ncdump -h $file | grep 'xi_v =' | cut -d ' ' -f3`
...
ncks -O -d eta_psi,0,$eta_psiM2 -d xi_psi,1,$xi_psiM1 -d xi_u,1,$xi_uM1 -d eta_v,0,$eta_vM2 $file $file
